  ...
  function render() {
    ...
    $('#main').innerHTML = `
      ...
      ${me ? `<section class="rank-hero" aria-label="Your rank">
        ...
        <div><div class="rank-hero__label">Why this position?</div><p style="font:400 16px/24px var(--font);margin:4px 0 12px">${me.summary.required - me.summary.accepted} required datasets are not yet accepted. ${me.summary.dq == null ? 'No rows validated yet.' : me.summary.dq + '% of validated rows passed.'} ${me.summary.late ? me.summary.late + ' arrived late.' : 'None arrived late.'}</p><a class="btn btn--outline btn--md" href="${R.role().team === 'inst' ? 'submissions.html?view=table' : 'monitor.html?inst=' + me.inst.id}">Review ${R.role().team === 'inst' ? 'my' : 'their'} datasets →</a></div>
      </section>` : ''}
