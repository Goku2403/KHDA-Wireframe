...
(function () {
  ...
  function cards(l) {
    ...
    return groups.filter(([, g]) => g.length).map(([name, g]) => `<div class="group-title">${esc(name)} <span class="count-badge">${g.length}</span></div><div class="inst-grid">${g.map(s => `<a class="inst-card" href="monitor.html?inst=${s.inst.id}">
  ...
  function renderSector() {
    ...
    $('#main').innerHTML = `
      <div class="page-head"><div><div class="page-head__eyebrow">KHDA Data · Monitor</div><h1 class="page-head__title">Institution monitor</h1><p class="page-head__sub">Every institution against every dataset · ${esc(per.label)}</p></div>
        <div class="page-head__actions"><a class="btn btn--outline" href="sector.html">← Sector home</a><a class="btn btn--outline" href="compliance.html">Compliance history</a></div></div>
      <div class="kpi-grid">
    ...
    $('#main').querySelectorAll('[data-view]').forEach(b => b.addEventListener('click', () => { view = b.dataset.view; history.replaceState(null, '', 'monitor.html?view=' + view); renderSector(); }));
    ...
    $('#main').querySelectorAll('[data-cell]').forEach(c => c.addEventListener('click', () => { const [id, sheet] = c.dataset.cell.split('|'); location.href = `monitor.html?inst=${id}&sheet=${encodeURIComponent(sheet)}`; }));
  ...
  function renderInst() {
    ...
    $('#main').innerHTML = `
      <div class="page-head"><div><div class="page-head__eyebrow">Institution monitor · ${esc(inst.waveLabel)}</div><h1 class="page-head__title">${esc(inst.name)}</h1><p class="page-head__sub">${esc(inst.location)} · Follow submissions and resolve outstanding work · ${esc(per.label)}</p></div>
        <div class="page-head__actions"><a class="btn btn--outline" href="monitor.html">← All institutions</a><a class="btn btn--outline" href="compliance.html?inst=${inst.id}">Compliance history</a><a class="btn btn--outline" href="remediation.html?inst=${inst.id}">Remediation report</a><button class="btn btn--primary" type="button" id="openEmail">✉ Send email</button></div></div>
      <div class="kpi-grid">
  ...
  function openEmail() {
    ...
    m.innerHTML = `<aside class="sheet sheet--wide" role="dialog" aria-modal="true" aria-labelledby="emailTitle"><div class="sheet__body">
      ...
      <div class="field-row"><label>Attach</label><label style="display:flex;gap:8px;align-items:center;font:400 16px/24px var(--font)"><input type="checkbox" id="emAttach" checked> Remediation report (${sm.needsCorrection} datasets · ${sm.rowsRejected.toLocaleString()} rows) as XLSX</label></div>
