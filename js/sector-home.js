  ...
  function render() {
    ...
    $('#main').innerHTML = `
      <div class="page-head"><div><div class="page-head__eyebrow">KHDA Data · ${esc(R.role().label)}</div><h1 class="page-head__title">Sector overview</h1><p class="page-head__sub">Submission progress and follow-up priorities · ${esc(per.label)}</p></div>
        <div class="page-head__actions"><a class="btn btn--outline" href="monitor.html?view=matrix">Compliance matrix</a><a class="btn btn--outline" href="remediation.html">Remediation report</a><a class="btn btn--primary" href="monitor.html">Open monitor</a></div></div>
      <div class="kpi-grid">
        <a class="kpi kpi--info" href="monitor.html"><div class="kpi__label">Total HEIs</div><div class="kpi__value">${sec.total}</div><div class="kpi__note">Institutions in scope · 4 onboarding waves</div><span class="kpi__link">Open monitor →</span></a>
        ...
        <a class="kpi kpi--primary" href="remediation.html"><div class="kpi__label">Rows to fix</div><div class="kpi__value">${sec.rowsRejected.toLocaleString()}</div><div class="kpi__note">Across ${sec.needsCorrection} dataset submissions needing correction</div><span class="kpi__link">Remediation report →</span></a>
      </div>
      <div class="grid-main-side">
        <section class="panel"><div class="panel__head"><div><h2 class="panel__title">Entity submissions</h2><p class="panel__sub">See who needs follow-up and open their dataset agenda. Reason codes come from the requirement table; SLA age from the last receipt.</p></div>
          ...
          <div class="table-wrap" style="border-radius:12px"><table class="data-table data-table--compact"><thead><tr><th></th><th>Entity</th><th>Outstanding</th><th>DQ score</th><th>Submissions missed</th><th>Contact &amp; next step</th></tr></thead><tbody>
            ${list.map(s => `<tr><td><input type="checkbox" data-sel="${s.inst.id}" aria-label="Select ${esc(s.inst.short)}"${selected.has(s.inst.id) ? ' checked' : ''}></td>
              <td><div class="cell-title"><a href="monitor.html?inst=${s.inst.id}">${esc(s.inst.name)}</a></div><div class="cell-sub" style="display:flex;gap:8px;align-items:center;margin-top:4px">${K.stateChip(s.state)}<span>${esc(s.reason)}</span></div><div class="cell-sub">${esc(s.inst.waveLabel)} · ${slaAge(s)}</div></td>
              ...
              <td><div class="cell-sub">👤 ${esc(s.inst.liaison)} · Owner: ${esc(s.inst.owner)}</div><div style="display:flex;gap:8px;margin-top:6px;flex-wrap:wrap"><a class="btn btn--outline btn--sm" href="monitor.html?inst=${s.inst.id}">View datasets →</a><a class="btn btn--outline btn--sm" href="monitor.html?inst=${s.inst.id}&email=1">✉ Send email</a><a class="btn btn--text btn--sm" href="compliance.html?inst=${s.inst.id}">⟲ History</a></div></td></tr>`).join('')}
          </tbody></table></div>
        ...
        <div style="display:flex;flex-direction:column;gap:24px">
          ...
          <section class="panel"><h2 class="panel__title">Follow-up queue · SLA</h2>${sec.institutions.filter(s => !s.compliant).sort((a, b) => (b.lastReceivedAt ? 0 : 1) - (a.lastReceivedAt ? 0 : 1) || (a.lastReceivedAt || 0) - (b.lastReceivedAt || 0)).slice(0, 5).map(s => `<div class="next-move"><span class="next-move__n">!</span><div><div class="next-move__title"><a href="monitor.html?inst=${s.inst.id}">${esc(s.inst.short)}</a> · ${esc(s.reason)}</div><div class="next-move__text">${slaAge(s)} · escalate to Supervisor after 14 days</div></div></div>`).join('')}</section>
        </div>
      </div>
    ...
    $('#bulkSend').addEventListener('click', () => { if (!R.can('supervisor')) { window.khdaToast('error', 'Supervisor tier required', 'Bulk communications are sent by a Supervisor or Administrator.'); return; } location.href = 'monitor.html?inst=' + [...selected][0] + '&email=1&bulk=' + selected.size; });
