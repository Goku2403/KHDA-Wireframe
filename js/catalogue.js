...
(function () {
  ...
  function tableHtml(list) {
    ...
    return order.filter(g => groups[g]).map((g, gi) => {
      ...
      return `<details class="cat-group" ${gi === 0 || groupBy === 'none' || ds.length <= 4 ? 'open' : ''}>
        ...
        <div class="cat-group__body"><table class="data-table data-table--compact" style="min-width:1000px"><thead><tr><th>Dataset</th><th>Frequency</th><th>Due</th><th>Submission status</th><th>Freshness</th><th>Reconciliation</th><th>Details</th></tr></thead><tbody>
  ...
  function openDetails(sheet, tab) {
    ...
    let html = '';
    if (detailsTab === 'spec') html = `<table class="data-table data-table--plain data-table--compact"><thead><tr><th>Field</th><th>Rule</th></tr></thead><tbody>${sc.fields.map((f, i) => `<tr><td><div class="cell-title">${esc(f.db)}</div><div class="cell-sub">${esc(f.label)}</div><div class="cell-code">Dictionary: ${esc(d.sheet)}, row ${i + 2}${f.required ? ' · required' : ''}</div></td><td><ul class="spec-rules">${ruleLines(f).map(r => `<li>${r}</li>`).join('')}</ul></td></tr>`).join('')}</tbody></table>`;
  ...
  // ---------- events ----------
  if ($('#catSearch')) $('#catSearch').addEventListener('input', e => { query = e.target.value.trim().toLowerCase(); page = 1; render(); });
